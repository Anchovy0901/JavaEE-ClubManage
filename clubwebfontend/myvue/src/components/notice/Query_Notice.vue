  ...
  >
    ...
    <el-main>
      ...
      >
        ...
        <el-table-column label="公告内容" prop="noticetcontent">
        </el-table-column>
        <el-table-column label="发布时间" prop="noticetime" sortable>
        </el-table-column>
        <el-table-column
          label="活动范围"
          prop="noticerange"
        ...
        >
          <template slot-scope="scope">
            <el-tag
              :type="
                scope.row.noticestate === '等待审核' ? 'danger' : 'success'
              "
              disable-transitions
              >{{ scope.row.noticestate }}</el-tag
            >
          </template>
        </el-table-column>
...
export default {
  ...
  mounted () {
    this.$axios.get('/allnotice').then((response) => {
